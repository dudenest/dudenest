...
void main() {
  group('email (login text)', () {
    test('accepts valid', () {
      ...
      expect(validateRhField('text', '  demo@example.com  '), isNull); // trimmed
    });
    test('rejects malformed', () {
      ...
      expect(validateRhField('text', '1demo@x.com'), isNotNull); // starts with digit
      expect(validateRhField('text', '.demo@x.com'), isNotNull); // starts with dot
      expect(validateRhField('text', 'demo@localhost'), isNotNull); // no dot in domain
    ...
    test('login may be a phone', () {
      ...
      expect(validateRhField('text', '+48 12'), isNotNull); // 4 digits — too short as phone
    });
  });

  group('password', () {
    test('only non-empty, no trimming/char rules', () {
      expect(validateRhField('password', 'x'), isNull);
      expect(validateRhField('password', '  spaces ok  '), isNull);
  ...
  group('phone (tel)', () {
    test('digits + country code', () {
      ...
      expect(validateRhField('tel', '+1 (415) 555-2671'), isNull); // formatting stripped
    ...
    test('rejects letters / too short / too long', () {
      ...
      expect(validateRhField('tel', '+1234567890123456'), isNotNull); // 16 digits
